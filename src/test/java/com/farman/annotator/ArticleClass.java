...
//            System.out.println(newText);
            ...
            newText = removeBetween("2010 Mathematics Subject Classification", "ZBL", newText, " ");
//            System.out.println(newText);
...
//            newText = removeBetween("<!-- ***", "<!-- ***", newText, " ");
            newText = removeBetween("<!-- *** native", "push({});", newText, " ");
            newText = removeBetween("<!-- *** buf", "<!-- *** text *** -->", newText, " ");
//            newText = rawText.replaceAll("<[^>]*>", " ");
//            System.out.println(newText);

            newText = newText.replaceAll("\\\\[n, r]", "");
...
//            System.out.println(newText);
        ...
        try {
            String path = ArticleClass.class.getClassLoader().getResource("").getPath() + "/" +"russianArticles-1.txt";
...
//            System.out.println("existing: "+index+" new: "+newItem);
    ...
    private static String removeBetween(String sub1, String sub2, String source, String replacement) {
...
//            System.out.println(current.substring(sub1Index, sub2Index));
            if (sub2Index > -1)
                result = current.substring(0, sub1Index) + replacement + current.substring(sub2Index+sub2.length(), current.length());
            else break;
...
//                keyIndex = (keyIndex + 1) % keys.size();
    ...
    public static void parseYandexLemmas() {
        String filePath = Constants.HOME.toString()+"englishArticlesRawLemmas.txt";
        List<String> articles = new ArrayList<>();
        try {
            BufferedReader in = new BufferedReader(new FileReader(filePath));
            String article;
        ...
        String resultFile = Constants.HOME.toString()+"englishArticlesLemmas.txt";
        try {
